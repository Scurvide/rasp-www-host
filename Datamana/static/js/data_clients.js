...
$(function() {
    ...
    let refreshTime = 500; // Refresh time for data reloads from database
    let rurl = '';
    ...
    $( '#clients a' ).on( 'click', function(event) {
        ...
        var clientName = splitted[splitted.length - 1];
        $( '#' + clientName + 'Client' ).css('background-color', 'grey');

        // Build measuring button actions for selected client
        $( '#measureActions' ).html(
            "<a id = 'getMeasurement' class = 'button'>Measure</a>" +
            "<a id = 'autoMeasure' class = 'button'></a>" );
        $( '#autoMeasure').load( '/measure/auto/' + clientName + '/status' );
        $( '#autoMeasure' ).on( 'click', function(event) {
            $( '#autoMeasure').load( '/measure/auto/' + clientName );
        });
        $( '#getMeasurement' ).on( 'click', function(event) {
            $.ajax({ url: '/measure/' + clientName });
        });
    ...
    function charterManager() {
        for (let u = 0; u < dataTables.length; u++) {
            charter( dataTables[u], dataTypes[u], dataUnits[u], dataGraphs[u] )
        }
    };

});

function charter( dataTable, dataType, dataUnit, dataGraph ) {
    ...
    if ( dataTable != {} && graph) {
        ...
        if ( dataGraph === 'point' && dataTable.length > 0 ) {
            ...
            let dataTypeName = dataType.charAt(0).toUpperCase() + dataType.slice(1);
            let handler = '#chartTable' + dataTypeName;
            let handlerTrLast = handler + ' tr:last';
            // Remove the table first
            $( handler ).remove();
            $( '#charts' ).append(
                "<table id = 'chartTable" + dataTypeName +
                "' class = 'chartTable'></table>"
            );
            // Ensure style before constructing the chart
            $( handler ).prepend("<link rel='stylesheet' type='text/css' href=" + chartCSS + ">");
        ...
        else if ( dataGraph === 'bar' && dataTable.length > 0 ) {
        ...
        // If dataGraph is not supported, empty the table
